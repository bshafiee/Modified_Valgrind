...
void* HG_zalloc ( const HChar* cc, SizeT n )
{
    void* p;
    tl_assert(n > 0);
    p = VG_(malloc)( cc, n );
...
HChar* HG_strdup ( const HChar* cc, const HChar* s )
{
...
{
    CountNode* md = HG_zalloc( "hg.new_CountNode", sizeof(CountNode) );
    tl_assert(md);
    VG_(memset)(md, 0, sizeof(CountNode));
    return md;
}
static void delete_CountNode ( CountNode* md )
{
    HG_free(md);
}

void freenode_function (CountNode* node)
{
    delete_CountNode(node);
}

static void lk_hash_init()
{
   tableLoad = VG_(HT_construct)("tableLoad");
   tableStore = VG_(HT_construct)("tableStore");
   tableModify = VG_(HT_construct)("tableModify");
}
...
static const HChar* clo_program_name = "unknown";
static const HChar* clo_output_dir = "unknown";
...
"    --pname=<name>             will be appended to the output file names\n"
"    --outputdir=<name>         The directory to output files in should finish with \\ \n"
...
static void add_load_node(Addr addr)
{
    ...
    if(existing == NULL)
    {
        ...
        node->key = addr;
        node->data = 1;
        VG_(HT_add_node)(tableLoad,node);
    }
    else//We have this node already!
    {
        ...
        {
            CountNode* next = existing->next;
            VG_(printf)("Collision Occured");
            while(next!=NULL)
            {
                if(next->key==addr)
                    next->data++;
                VG_(printf)("Collision Resolved in load table");
                break;
            }
        }
    }
...
static void add_store_node(Addr addr)
{
    ...
    if(existing == NULL)
    {
        ...
        node->key = addr;
        node->data = 1;
        VG_(HT_add_node)(tableStore,node);
    }
    else//We have this node already!
    {
        //Collision possibility
...
static void add_modify_node(Addr addr)
{
    ...
    if(existing == NULL)
    {
        ...
        node->key = addr;
        node->data = 1;
        VG_(HT_add_node)(tableModify,node);
    }
    else//We have this node already!
    {
        //Collision possibility
